...
string-matching generic exceptions. All extend :class:`TrustLineError`, so
``except TrustLineError`` covers everything the SDK raises.
...
class TrustLineError(Exception):
    """Base class for every error the TrustLine SDK raises."""
...
class MaxDrawExceededError(TrustLineError):
