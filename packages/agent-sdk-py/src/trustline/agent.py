"""``TrustLineAgent`` -- the interface an AI agent uses to take and repay
...
Stellar key, so the whole lifecycle is agent-driven::
    ...
    tl = TrustLineAgent(secret, api_base_url=..., contracts=...)
    tl.register()
...
class TrustLineAgent:
    ...
    def ensure_contracts(self) -> Dict[str, str]:
        ...
        if not (c["registry"] and c["creditLine"] and c["vault"]):
            raise ValueError(
                "TrustLine contract ids unavailable from /config -- pass them via "
                "contracts={...}."
            )
        self._contracts = c
        return c
    ...
    def pay_with_credit(
        ...
        *,
        ...
        :param max_draw: optional cap on how much credit a single call may draw
                         (raises :class:`~trustline.errors.MaxDrawExceededError`)
        """
